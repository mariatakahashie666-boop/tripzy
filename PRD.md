...
**Functionality**: AI scans documents using GPT-4o vision to extract traveler information, then displays extracted data in editable form for user verification. Uses advanced OCR to read passport MRZ zones and flight ticket details with high accuracy. Includes multi-leg flight support with dedicated interface for adding connecting flights and transit countries.
**Purpose**: Ensure accuracy before generating documents, giving users control and catching AI mistakes. Extracts names EXACTLY as they appear on documents. Captures transit country information for comprehensive requirements analysis.
**Trigger**: After document upload completes.
**Progression**: AI processes images (10-20 seconds) → GPT-4o analyzes passport photo and MRZ zone → Extracts flight details from ticket → Display extracted fields (name, passport, DOB, nationality, flight details) → User can toggle "Multi-Leg Flight" option → If enabled, user adds flight legs with transit countries, dates, flight numbers, and layover durations → Show warning "Please check carefully" → Show confidence score (if <90%, highlight uncertain fields) → User edits any incorrect fields → User confirms "Yes, Proceed"
**Success Criteria**: All critical fields extracted with 95%+ accuracy, names match passport exactly, users can edit any field, multi-leg flights properly captured with transit countries, confidence score displayed, confirmation required before proceeding.

### Requirements Analysis & Checklist
**Functionality**: AI determines required documents based on origin, destination, nationality, and transit countries, then displays comprehensive checklist. CRITICAL: AI must analyze all three factors (nationality, current location/departure, and destination) plus any transit countries before determining requirements. For multi-leg flights, analyzes each transit country for visa requirements considering layover duration and nationality-specific exemptions. Philippines eTravel is ONLY required for passengers departing FROM the Philippines.
**Purpose**: Show travelers exactly what they need including transit visa requirements, preventing surprises at the airport and avoiding incorrect document requirements.
**Trigger**: User confirms extracted data accuracy.
**Progression**: AI analyzes nationality + origin + destination + transit countries → Determines country-specific requirements based on departure point and each transit → Display categorized checklist (Exit Documents, Entry Documents, Transit Documents, Physical Requirements, Optional) → Transit section shows connecting country requirements with layover duration context → Show affiliate pop-ups (eSIM, hotel, tours) → User ticks what they already have → Click "Proceed to Payment"
**Success Criteria**: Checklist accurately reflects country-specific requirements based on all factors including transit countries, transit visa requirements properly assessed based on layover duration and nationality exemptions, users can mark items they possess, affiliate offers displayed contextually, no false requirements shown.
...
**Multi-leg Flights** - System detects connecting countries through dedicated multi-leg flight interface in data verification, adds +$2 per transit country, generates transit visa requirements for each connection. Users can add multiple flight legs with transit duration info, AI analyzes layover duration and nationality to determine if transit visa is required (short airside layovers <24h may not need visa). Transit requirements displayed in separate "Transit Documents" section with connecting country information.
